"""Production environment settings — Strict fail-closed validation."""
...
if not SECRET_KEY or "insecure" in SECRET_KEY.lower() or len(SECRET_KEY) < 20:
    raise ImproperlyConfigured(
        "DJANGO_SECRET_KEY environment variable is mandatory and cannot be an insecure default in production."
    )

if not os.environ.get("DATABASE_URL"):
    raise ImproperlyConfigured(
        "DATABASE_URL environment variable is mandatory in production. SQLite fallback is prohibited."
    )

raw_hosts = os.environ.get("ALLOWED_HOSTS", "")
if not raw_hosts or "*" in raw_hosts:
    raise ImproperlyConfigured(
        "ALLOWED_HOSTS must be explicitly configured without wildcards in production."
    )
