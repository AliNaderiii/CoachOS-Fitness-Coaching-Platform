...
def connect(request):
    return Response(
        {"authorization_url": "/mock/oauth/authorize?provider=mock_fitness&scope=read_activity"},
...
def callback(request):
    return Response(
        {"connection_state": "connected", "message": "Mock authorization completed."},
        status=status.HTTP_200_OK,
    )
...
def sync(request, connection_id):
    ...
    return Response(
        {"events_imported": len(events), "cursor_updated": True}, status=status.HTTP_200_OK
    )


@api_view(["GET"])
@permission_classes([])
def connection_status(request, connection_id):
    return Response(
        {
...
def provenance(request, connection_id):
    return Response(
        {
            "provider_timestamp": "2026-08-09T00:00:00Z",
            "provider_event_id": "mock_event_001",
            "imported_at": "2026-08-16T10:30:00Z",
        },
        status=status.HTTP_200_OK,
    )
...
def disconnect(request, connection_id):
    return Response(
        {"connection_state": "disconnected", "revocation_status": "pending"},
        status=status.HTTP_200_OK,
    )
