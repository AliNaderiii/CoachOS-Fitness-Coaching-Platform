...
| **Tier 3: Sensitive Health-Adjacent Data** | Physiological metrics, physical limitations, subjective exertion, and readiness logs. | Body weight, body fat %, pain flags, injury history, fatigue ratings, check-in notes. | **Strictly Confidential.** Field-level encryption options; zero logging in web server debug logs; audited reads. | **Assigned Coach Only** via active assignment; Athlete (self); Org Owner (aggregate reporting only; raw individual notes require audited escalation). |
| **Tier 4: Progress Media Assets** | Visual body conditioning photographs and private form check videos. | Athlete front/side/back progress photos, technique review clips. | Private S3 object storage; **never public**; access strictly via time-limited (<= 15 min) cryptographically signed URLs. | **Explicit Athlete Consent Required**; Assigned Coach; Athlete (self); Org Owner and Platform Admin blocked unless audited break-glass escalation. |
