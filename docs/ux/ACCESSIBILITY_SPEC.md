# Accessibility Specification (WCAG 2.2 AA Design Target) — CoachOS

**Document version:** 1.0.0 (Phase 02 Baseline)  
**Last updated:** 2026-08-10  
**Design Target (not certification):** Web Content Accessibility Guidelines (WCAG) 2.2 Level AA — proposed pattern; requires implementation validation and accessibility testing  
**Supported Locales:** Persian (`fa-IR`, RTL) and English (`en-US`, LTR)  
**Design Phase:** Phase 02 — Documentation and specifications only. No application source code. All WCAG statements below are design targets, not compliance claims.
...
1. **Perceivable (design target; requires testing):**
   - Text contrast ratio design target >= **4.5:1** for standard text (< 18pt) and >= **3:1** for large text (>= 18pt bold or >= 24pt regular) against its background.
   - UI controls, active borders, and focus rings design target >= **3:1** contrast against adjacent surfaces.
...
2. **Operable (proposed pattern; requires implementation validation):**
   - 100% of interactive elements are designed to be reachable and operable via keyboard alone (`Tab`, `Shift+Tab`, `Enter`, `Space`, `Arrow Keys`, `Escape`) — requires keyboard testing.
   - Focus outline is designed to be prominently visible across all interactive components (2px solid `--color-border-focus` with 2px offset).
   - Touch targets: minimum **44×44 CSS pixels** per WCAG 2.5.5; **48×48px preferred design target** for primary CTAs and set checkmarks — actual implementation must be tested against accessibility and device usability requirements.
3. **Understandable:**
...
| **Athlete Today View** | Exercise cards have semantic headings (`<h2>`); CTA button has 48×48px preferred hit area (minimum 44×44px; requires implementation testing); video clips provide text coaching cues. | 1.3.1, 1.4.3, 2.5.5, 1.2.2 | Phase 07 / 13 |
