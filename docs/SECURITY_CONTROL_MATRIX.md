...
| T02 Session Theft + Frontend Secret Exposure (Corrected) | NFR-SEC-04 | HttpOnly; Secure; SameSite=Lax cookies (recommended MVP cookie sessions per ADR-032 corrected: HttpOnly true Secure true SameSite Lax CSRF double-submit/Django middleware), TLS1.3 HSTS, short-lived access ≤15min + rotating refresh in HttpOnly cookie reuse detection revoking all if reuse, explicit prohibition no long-lived tokens in localStorage, Frontend MUST NEVER access Secrets Manager directly — private secrets (DB URL, Django SECRET_KEY, Redis/S3 credentials, email API keys, JWT signing) only to backend/worker via server-side injection, frontend only public NEXT_PUBLIC_* runtime config (FE --> SecretMgr forbidden removed), CSP production preferred nonce/hash-based script-src no unsafe-inline as accepted production, temporary unsafe-inline exception marked with risk + TODO-CSP-001 hardening, bundle secret scan CI | Phase05 | Unit cookie flags (HttpOnly Secure SameSite Lax), Integration refresh reuse → revoke all sessions, E2E no token in localStorage, Security scan CSP nonce/hash preferred + bundle secret scan frontend must not contain private secrets, Integration FE has no access to SecretMgr | `authorization` middleware, `OPENAPI.yaml` cookieAuth + bearerAuth recommendation, `OBSERVABILITY.md`, `DEPLOYMENT_ARCHITECTURE.md` corrected secrets boundary + CSP, `CONTAINER_ARCHITECTURE.md` corrected | Proposed |
...
| T09 Stored XSS in Notes/Messages | NFR-SEC-06, FR-MSG-01, US-MSG-001 | Backend output encoding (escape HTML), frontend defense-in-depth DOMPurify sanitization, HttpOnly cookies prevent token theft even if XSS, CSP production preferred nonce/hash-based script-src (no unsafe-inline as accepted production per corrected DEPLOYMENT_ARCHITECTURE.md), temporary unsafe-inline exception marked temporary with risk + TODO-CSP-001 hardening, no dangerouslySetInnerHTML without sanitization, frontend never gets private secrets | Phase06/08 | Unit sanitization, E2E injection payload rendered as text, Security scan CSP nonce/hash preferred + header validation | `COMPONENT_BOUNDARIES.md` corrected, `OBSERVABILITY.md` Sentry, `DEPLOYMENT_ARCHITECTURE.md` corrected CSP | Proposed |
