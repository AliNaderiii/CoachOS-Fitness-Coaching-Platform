...
| **Persian Normalization** | استانداردسازی نویسه‌های فارسی | Algorithmic Unicode normalization folding Arabic script character variants (e.g., Arabic Yeh `ي` / `ى` → Persian `ی`, Arabic Kaf `ك` → Persian `ک`, Arabic digits `٠-٩` → Persian `۰-۹` or Latin `0-9`) to ensure reliable search matching. |
